...
std::string SHA1::final() {
    const uint64_t total_bits = (transforms * 64 + buffer.size()) * 8;
    ...
    // 최종 20바이트 바이너리 해시를 얻기 위해 수정
    std::string hash_str;
    hash_str.reserve(20);
    for (int i = 0; i < 5; i++) {
        hash_str += static_cast<char>((digest[i] >> 24) & 0xFF);
        hash_str += static_cast<char>((digest[i] >> 16) & 0xFF);
        hash_str += static_cast<char>((digest[i] >> 8) & 0xFF);
        hash_str += static_cast<char>((digest[i] >> 0) & 0xFF);
    }
    return hash_str;
}
